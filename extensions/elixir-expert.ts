...
export default function elixirExpertExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    try {
      const session = await getExpertSession(ctx.cwd);
      await session.start();
      ctx.ui.setStatus("pi-expert", "🔮 \x1b[35mExpert connected\x1b[0m");
    } catch {
      ctx.ui.setStatus("pi-expert", "🔮 \x1b[31mExpert failed\x1b[0m");
    }
  });
